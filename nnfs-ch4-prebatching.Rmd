...
train_model = function(model, inputs, y_true, epochs,
  ...
  if (!is.null(batch_size)){
    ...
    train_steps = ncol(inputs) %/% batch_size	 #(integer division)
    ...
    if (train_steps*batch_size < ncol(inputs)){
      train_steps = train_steps + 1
    }
    # Now we repeat this process for the validation data
    if (!is.null(validation_X)){
      validation_steps = ncol(validation_X) %/% batch_size
      if(validation_steps*batch_size < ncol(validation_X)){
 ...
 for(step in 1:train_steps){
  #Deal with batches:
   ...
   ##otherwise slice a batch
     ...
     if (step != train_steps){
       batch_X = inputs[(step-1)*batch_size:step*batch_size, ]
       batch_y = y_true[(step-1)*batch_size:step*batch_size, ]
     } else {
       ##for the last step, need to index precisely to the last row
       batch_X = inputs[(step-1)*batch_size:ncol(inputs), ]
       batch_y = y_true[(step-1)*batch_size:ncol(inputs), ]
     }
   }
...
if (length(baselayers) > 1){
  ...
  ##Returns
  list(final_metrics=final_metrics, validation=validation_metrics,
       metrics=metrics, 
...
Let's test our last categorical model:
