...
Now I test the sigmoid activation function and binary crossentropy loss function. Note that (for each sample) we technically just need one neuron in the output layer, since the sigmoid function will (eventually) be predicting 0 or 1. But we will still leave 3 output neurons, giving each an opportunity to predict the label.  
...
train = list(
  initalize = function(n_layers = NULL){
  ...
  add_activation =  function(model, activ_fn = c("linear", "relu", "sigmoid",
   ...
   # prior_layer = paste0("layer", length(model)-1)
   #if (model[[paste0(prior_layer)]]$class == "layer_dropout"){
   #    base_layer = length(model) - 2
   #  } else base_layer = length(model) - 1
    base_layer = pos-1
  ...
  add_loss = function(model, loss_fn=c("binary_crossent", "mae", "mse")){
...
```{r}
compile = function(model, inputs, y_true, epochs,
                   optimizer = c("adam"), 
  ...
  layer_parameters = list() 
    ##reset this list every time to overwrite the past params
  
  # Initialize random params for every layer
  ## First need to derive param settings from the chosen optimizer
  if (optimizer=="adam"){
    cache_set=TRUE
    momentum_set=TRUE
  } else if (optimizer=="sgd"){
    if (optim_args$momentum==TRUE) momentum_set=TRUE else momentum_set=FALSE
    cache_set=FALSE
  }
  ...
  for (b in baselayers){
    ...
    random_params = init_params(n_inputs = model[[current_layer]]$n_inputs,
                                ...
                                cache = cache_set)
    lb_params = (random_params)

    layer_parameters = c(layer_parameters, "layer" = list(lb_params))
...
for (epoch in 1:epochs){
  ...
  ## Next Layers
  for (b in 1:length(baselayers[-1])){##for all other base layers
    
    prior_baselayer = baselayers[b] ##index of prior baselayer (includ. 1st)
    input_layer = paste0("layer",prior_baselayer)
    current_layer = paste0("layer",baselayers[b+1])
    
    dense_b = layer_dense$forward(inputs = layers[[input_layer]]$dense$output,
    ...
    #activ_fn = model[[paste0("layer",b+2)]]$class ##not working
      ##current layer+1 since activation follows dense. current is already b+1
    ...
    names(layers)[[length(layers)]] = paste0("layer",current_layer)
  }
  
  ## Calculate Loss
  loss_fn = model[[length(model)]]$class ##last layer should be a loss layer
  
  loss_mse = loss_MeanSquaredError$forward(
    y_pred = layers[[length(layers)]]$output,
    y_true = y_true
    )
  
  ## Calculate any other metrics
  loss_mae = loss_MeanAbsoluteError$forward(
  ...
  layers_back = list()
  # Backward Pass
  ## First Backprop Layer (which is the last layer in the training sequence)
  ...
  ###Then backprop the dropour layer, IF dropout was used
  if (model[[baselayers[length(baselayer)]]]$dropout_rate == 0){
    dropout1 == FALSE 
    } else dropout1 == TRUE
  
  if (dropout1 == TRUE){ ##backprop dropout layer only if needed
  ...
  if (dropout1==TRUE){ d_layer_dense1=dropout_backprop1
  } else d_layer_dense1=activ_backprop1
  
  ###Finally, backprop the dense layer
  dense_backprop1 = layer_dense$backward(d_layer = d_layer_dense1,
                                        layer = layers[[length(layers)]]$dense)
  
  layer_b = list(dense_backprop=dense_backprop1)
  layers_back = c(layers_back, "layer_last" = list(layer_b))
  
  
  ## Next Layers
  for (b in baselayers[-length(baselayers)]){ ##for all layers but the last
    current_layer = paste0("layer", b)
    
    activ_backprop = activation_Linear$backward(d_layer = dense_backprop)
    
    if (model[[baselayers[b]]]$dropout_rate == 0){
    dropout_b == FALSE 
    } else dropout_b == TRUE
    
   if (dropout_b == TRUE){ ##backprop dropout layer only if needed
    dropout_backprop = layer_dropout$backward(
      d_layer = activ_backprop, 
      layer_dropout = layers[[current_layer]]$dropout
      )
    d_layer_dense_b=dropout_backprop
    } else d_layer_dense_b=activ_backprop
    
    dense_backprop = layer_dense$backward(d_layer = d_layer_dense_b,
  ...
  for (b in baselayers){
    ...
    optimal_params_b = optimizer_Adam$update_params(
                        ...
                        layer_backward = layers_back[[current_layer]],
                        learning_rate = optim_args$learning_rate,
                        lr_decay = optim_args$lr_decay, iteration = epoch,
                        epsilon = optim_args$epsilon,
                        beta_1 = optim_args$beta_1,
                        beta_2 = optim_args$beta_2
                        )
                      
    optim_b = list(optimal_params_b)
    layer_parameters = c(layer_parameters, "layer" = list(optim_b))
    names(layer_parameters)[[length(layer_parameters)]] = paste0("layer",b)
  }
...
model = train$initalize()
model = train$add_dense(model, n_inputs = 1, n_neurons = 128, 
                        dropout_rate = 0)
model = train$add_activation(model, activ_fn = "linear")
model = train$add_dense(model, n_inputs = 128, n_neurons = 64,
                        dropout_rate = 0)
model = train$add_activation(model, activ_fn = "linear")
model = train$add_loss(model, loss_fn = "mse")

options(error=recover); debug(compile)
compile(model, inputs = X, y_true = y, epochs = 200,
        optimizer = "adam", learning_rate = 0.01, lr_decay = 1e-3,
        epsilon = 1e-7)
...
if exists l1_params, l1_params=l1_params
else l1_params = init_params()
