...
if (length(baselayers) > 1){
  ...
  if (epoch %in% seq(0,epochs,by=100)){
   
   if (loss_fn=="categorical_crossentropy"){
     loss = loss_layer$loss
   } else {
     loss = mean(loss_layer$sample_losses)
   }
   report = c(epoch, loss)
  ...
  if (loss_fn=="categorical_crossentropy"){
    predictions = max.col(layers[[length(layers)]]$output$output,
                                          ties.method = "random")
  } else {
    if (ncol(layers[[length(layers)]]$output$output) > 1){
      ##if using multiple output neurons for a regression or binary problem...
      ##this is temporary, may want to change
      predictions = rowMeans(layers[[length(layers)]]$output$output)
    } else predictions = layers[[length(layers)]]$output$output
  }
  
  ##Returns
  list(final_metrics=final_metrics, metrics=metrics, predictions=predictions)
}
...
First, a binary problem where we'll use a sigmoid function for the final activation function and binary crossentropy for a loss function. Piping makes building a model even easier. 
```{r}
...
Now let's go back to multi-class classification, where we'll use a softmax function for the final activation function and categorical crossentropy for a loss function (we'll use our combined softmax activation + categorical crossentropy function as before).
...
Next: add regularization into the training function, then add a testing function.  
