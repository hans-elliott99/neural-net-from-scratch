...
TODO: 
- test out the test_model fn - I updated it so that if y_true is not given, the loss/metric calc stage is skipped and the fn is just used to generate predictions  
 - *need to add that update and also the updated softmax_crossentropy fn to pkg*
 - 
...
classification_assessment = classess(truths = spiral_data$label,
                                     predictions = fit_categ$predictions)
...
```
Note that the accuracies are different than if we just looked at the proportion of correctly guessed labels, which is what the `train_model()` function does to calculate overall accuracy:
```{r}
# sum of where labels = predicted labels / total labels
sum(spiral_data$label == fit_categ$predictions)/length(spiral_data$label)
```
...
If we are predicting onto new, unlabeled data, we can simply ignore the y_test argument and test_model() will still provide predictions:

```{r}
##Predict onto "unlabeled" data
predict = test_model(model = categ_model,
                     trained_model = fit_categ,
                     X_test = X_test)
predict$predictions


plot(x = X_test, col = predict$predictions)
```
