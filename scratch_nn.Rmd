...
Keep in mind that exponentiation can result in some very large numbers and we could run into an overflow error. For example, compare `exp(2)` to `exp(5)`.   
...
activation_Softmax = list(
  forward = function(inputs){
          ...
          # exponetiate
          #NOTE: I add a small decimal here since exp_values will be a 
          #denominator, and cannot divide by zero. This is a problem I actually
          #ran into before changing
          exp_values = exp(scaled_inputs) + 1e-7
          # normalize
...
Worth noting that the resulting matrix is known as a _Jacobian Matrix_, which is a matrix of partial derivatives in all of the combinations of both input vectors. In the book, the show how to implement these steps into the backpropagation, but also reveal a faster & easier to code method.
...
activation_Softmax = list(
  forward = function(inputs){
          ...
          exp_values = ifelse(exp(scaled_inputs)<=1e-7,1e-7,
                            ifelse(exp(scaled_inputs>=709),709,exp_values)
          # normalize
...
output = 
  activation_loss_SoftmaxCrossEntropy$forward(inputs = layer2$output,
      ...
      #metrics:
      LOSS = output$loss
      PRED = max.col(output$softmax_output) 
      ACC = mean(predictions==spiral_data$label)
...
#        paste0("epoch: ",epoch,  ", acc: ",accuracy, ", loss: ",loss)}
...
for (epoch in 1:tot_epochs){
 if (epoch %in% seq(0,tot_epochs,by=100)){
        print(epoch)}#  "acc: ",accuracy, "loss: ",loss)}
